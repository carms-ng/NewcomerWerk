...
# Load Montreal street name
mtl_st_url = "https://en.wikipedia.org/wiki/Category:Streets_in_Montreal"
mtl_st_html_file = open(mtl_st_url).read
mtl_st_html_doc = Nokogiri::HTML(mtl_st_html_file)

mtl_addresses = mtl_st_html_doc.search('.mw-category li').map do |item|
  pattern = /^(.+)((\s\(|,|\s—).+)$/

  if item.text.strip.match?(pattern)
    match_data = item.text.strip.match(pattern)
    address = match_data[1]
  else
    address = item.text.strip
  end + ', Montreal, Quebec, Canada'
end
